...
const YOGAWEEKS: [[[ActivityType; 6]; 9]; 4] = [
    [
        [
            ActivityType::Skip,
...
const YOGAWEEKSRECOVERY: [[[ActivityType; 6]; 3]; 4] = [
    [
        [
            ActivityType::Skip,
            ActivityType::StrengthEmpty,
...
impl YogaLevel {
    ...
    fn index(self) -> usize {
        match self {
            YogaLevel::NotSet => 0,
            YogaLevel::Beginer => 1,
            YogaLevel::Intermediate => 2,
            YogaLevel::Advanced => 3,
        }
    }
    ...
    pub fn activities(&self, week: usize) -> &[ActivityType; 6] {
        &YOGAWEEKS[self.index()][week]
    }
    ...
    pub fn recovery(&self, week: usize) -> &[ActivityType; 6] {
        &YOGAWEEKSRECOVERY[self.index()][week]
    }
}

const STRENGTHWEEKS: [[[ActivityType; 2]; 9]; 6] = [
    [
...
const STRENGTHWEEKSRECOVERY: [[[ActivityType; 2]; 3]; 6] = [
    [
        [ActivityType::Skip, ActivityType::Skip],
        [ActivityType::Skip, ActivityType::Skip],
        [ActivityType::Skip, ActivityType::Skip],
...
impl StrengthLevel {
    ...
    pub fn activities(&self, week: usize) -> &[ActivityType; 2] {
        &STRENGTHWEEKS[self.index()][week]
    }
    ...
    // with 2 strength activities per week.
    pub fn recovery(&self, week: usize) -> &[ActivityType; 2] {
        &STRENGTHWEEKSRECOVERY[self.index()][week]
    }
}
        ...
        match week_type {
            WeekType::Active => {
                week_activities = self.combine_activities(
                    self.yoga_level.activities(self.week_index),
                    self.strength_level.activities(self.week_index),
                );
                self.advance_week_index()
            }
            WeekType::Recovery => {
                week_activities = self.combine_activities(
                    self.yoga_level.recovery(self.recovery_index),
                    self.strength_level.recovery(self.recovery_index),
                );
